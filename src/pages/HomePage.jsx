...
// Added by Musaddik — AI hooks and imports
  ...
  return (
    <div className="absolute bottom-4 left-4 z-20 w-[min(286px,calc(100%-2rem))] rounded-2xl border border-[#D9D4CE] bg-[#FFFDFC] p-2.5 shadow-[0_18px_44px_rgba(22,22,22,0.16)] transition duration-200 hover:scale-[1.025] hover:shadow-[0_24px_60px_rgba(22,22,22,0.22)] sm:left-auto sm:right-4">
      ...
      <div className="mt-2 flex items-center justify-between gap-3">
        ...
        <button className="cursor-pointer rounded-full bg-[#B4232C] px-3 py-1.5 text-[11px] font-black text-white transition-[transform,background-color,box-shadow] duration-200 ease-out hover:-translate-y-0.5 hover:scale-105 hover:bg-[#7D1820] hover:shadow-[0_10px_22px_rgba(180,35_44,0.22)]">
          Details
        </button>
      </div>
    </div>
...
// Added by Musaddik
// Wraps Sami's map panel. Feeds dynamic AI recommendations states into the MoodMap component.
function MapPanel({ aiPins, onSelectPlace, mapCenter, selectedPlace, onClearSelectedPlace }) {
  return (
    <div className="relative min-h-[520px] overflow-hidden rounded-[26px] border border-[#D9D4CE] bg-[#EDE7DF] shadow-[0_24px_58px_rgba(22,22,22,0.14)] lg:h-full lg:min-h-0">
      {/* Dynamic Leaflet Map containing Sami's markers + Musaddik's AI recommendations */}
      <MoodMap aiPins={aiPins} onSelectPlace={onSelectPlace} mapCenter={mapCenter} />
      ...
      {/* Render Sami's static UI markers on top of the map frame */}
      ...
      {/* Render Sami's custom SelectedPlaceCard details */}
...
// Added by Musaddik
// Integrated HomePage combining Sami's premium UI layout with Musaddik's Gemini AI Moodie chatbot features.
export default function HomePage() {
  ...
  // Auto-close dialog after locations successfully load
  useEffect(() => {
  ...
  // Added by Musaddik — recommendation API call
  const handleFetchRecommendations = async (moodQuery) => {
    ...
    try {
      ...
      // Re-center Leaflet focus to first query result coordinate
  ...
  return (
    <section className="mx-auto flex w-full max-w-[1500px] flex-col px-4 pb-4 pt-3 sm:px-6 lg:h-[calc(100vh-64px)] lg:min-h-[640px] lg:overflow-hidden relative">
      
      {/* Added by Musaddik — AI Search Tags Banner */}
      ...
      {/* Added by Musaddik — Error boundary display */}
      {error && (
        <div className="mb-2 bg-red-50 text-red-700 text-xs font-semibold px-4 py-2 rounded-xl border border-red-200">
          ⚠️ {error}
        </div>
      )}
      ...
      <div className="grid flex-1 gap-4 overflow-hidden lg:grid-cols-[330px_minmax(0,1fr)] relative">
        ...
        {/* Map Panel integration wrapping Leaflet maps */}
      ...
      {/* Added by Musaddik — Floating full-screen dialogue overlay */}
      <MoodInputModal
        isOpen={isMoodieOpen}
